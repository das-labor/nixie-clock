...
#ifndef SPI_SS_MOSI_PORT
	#define SPI_SS_MOSI_PORT SPI_PORT
#endif

#ifndef SPI_SS_MOSI_DDR
	#define SPI_SS_MOSI_DDR SPI_DDR
#endif
...
void spi_init()
{
	...
	SPI_SS_MOSI_DDR |= _BV(SPI_PIN_MOSI) | _BV(MCP_CS_BIT);
	SPI_DDR |= _BV(SPI_PIN_SCK);
#else
	/* configure MOSI, SCK, lines as outputs */
...
uint8_t spi_send(uint8_t data)
{
	...
	for (x = 0; x < 8; x++) {
		if (data & 0x80) {
			SPI_SS_MOSI_PORT |= (1 << SPI_PIN_MOSI);
		} else {
			SPI_SS_MOSI_PORT &= ~(1 << SPI_PIN_MOSI);
		}
